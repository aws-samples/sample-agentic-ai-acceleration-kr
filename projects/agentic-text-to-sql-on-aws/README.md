...
**코어 질의 파이프라인**
- **자연어 → SQL → 결과 스트리밍**: intent → schema linking → SQL 생성 → AST 검증 → 실행 → 요약의 Strands Graph 흐름. 실행 실패는 오류를 되먹여 재생성하는 self-correction 루프로 복구합니다.
- **clarification(재질의)**: 모호한 질의(기간·지표 미지정 등)는 Strands interrupt → AG-UI `CUSTOM(clarification_request)` 이벤트 → CopilotKit 폼(select/date-range/text) → 같은 세션 재호출로 중단 지점부터 재개합니다.
- **멀티 데이터 소스**: `run_sql(sql, datasource="aurora"|"redshift")` 라우팅. Aurora PostgreSQL(RDS Data API)과 Redshift Serverless(Redshift Data API, 비동기 폴링) 모두 read-only 사용자로 실행합니다.

**Semantic layer (비즈니스 컨텍스트)**
- **DynamoDB system-of-record** (`semantic-layer/`): 비즈니스 용어·few-shot·스키마 메타의 CRUD와 항목 단위 버전 관리(`v0` 최신본 + 이력), `candidate`/`published`/`rejected` 상태 분리 — 미승인 지식은 에이전트에 노출되지 않습니다(지식 오염 방어선).
...
- **Neptune Serverless 그래프**: `(Table)-[:JOINS]->(Table)`, `(Term)-[:MAPS_TO]->(Column)` — schema linking 시 join-path 순회(GraphRAG).

**도구 평면·보안**
- **AgentCore Gateway 단일 도구 평면**: MCP 서버 3개(sql-execution / semantic-retrieval / datasource-admin — 모두 AgentCore Runtime 호스팅)를 Gateway MCP target으로 집약(아웃바운드 SigV4, semantic tool search 활성). orchestrator는 `TOOL_PLANE_MODE=gateway`로 경유합니다(직접 연결 `direct`는 폴백).
- **Identity(인바운드 JWT) + 사용자 On-Behalf-Of**: Gateway가 Cognito JWT(customJWTAuthorizer)를 검증합니다. admin panel은 사용자 AccessToken을 그대로 Gateway MCP에 전달해 Cedar가 실제 사용자 그룹으로 인가하고, orchestrator는 `forwardedProps.userAccessToken`이 오면 사용자 토큰으로, 없으면 Cognito M2M 서비스 계정으로 도구를 호출합니다.
- **Cedar Policy ENFORCE**: PolicyEngine(default-deny + forbid-wins). 일반 사용자는 `run_sql`/`search_schema`만 허용(action 스코프), 관리 도구는 Manager/Admin 전용 — 미인가 도구는 tools/list에서 아예 제외됩니다.
- **READ-ONLY 4중 방어**: ① Cedar default-deny ② SQLGlot AST allow-list(LLM 밖, 소스별 dialect, UNLOAD/COPY 차단) ③ 최소 권한 IAM ④ DB SELECT-only grant.

**Admin panel (거버넌스 콘솔)**
- `admin/` (ECS Fargate + 전용 ALB): Cognito 로그인 → `aws-jwt-verify` 검증 → `cognito:groups`로 화면·API 인가(Manager/Admin 분리). semantic 큐레이션 · 승인 큐(승인/반려 — 반려 사유 이력, 재승인 가능) · 데이터소스 등록/테스트/스키마 크롤 · 사용자·그룹 관리 + Cedar 정책 뷰 · 메트릭/트레이스 탐색 · 평가·개선 화면.
- **쓰기 경로 단일화**: admin web은 DynamoDB를 직접 쓰지 않습니다. 모든 큐레이션·승인은 사용자 JWT → Gateway MCP → `datasource-admin-mcp` 경로로만 흐릅니다.

**개선 파이프라인 (평가 → 개선 → 승인 반영)**
- **Execution Accuracy(EX) 평가** (`evaluation/`): gold NL→SQL 데이터셋(`goldset-v1.jsonl`) 기반 custom code-based evaluator. 트레이스에서 (질문, 생성 SQL)을 복원해 goldset과 매칭하고, gold/생성 SQL을 read-only로 각각 실행해 정규화 결과셋을 비교합니다(LLM judge 비용 없음). ※ 이 evaluator만 Lambda — AgentCore Evaluations의 code-based evaluator 서비스 규격이며, "Tool layer Lambda 금지" 원칙의 명시적 예외입니다.
- **Online/batch evaluation**: orchestrator 트레이스를 상시 샘플링 평가(builtin Correctness·ToolSelectionAccuracy + EX)하고, admin panel에서 배치 평가 실행·결과 리뷰·개선 추천(시스템 프롬프트/도구 설명) 생성을 수행합니다.
...
- **도구 노출 모델**: 모든 도구는 AgentCore Runtime에 MCP 서버로 호스팅되고(Lambda 미사용), AgentCore Gateway의 MCP target으로 등록되어 Gateway가 유일한 도구 평면이 됩니다.
...
│       ├── semantic-stack.ts      # DynamoDB/Neptune Serverless/OSIS/graph-sync Lambda
│       ├── runtime-stack.ts       # AgentCore Runtime × 4 (orchestrator + MCP 3)
│       ├── gateway-stack.ts       # AgentCore Gateway + MCP target 3 + Cedar PolicyEngine
│       ├── ui-stack.ts            # ECS Fargate + 퍼블릭 ALB
│       ├── admin-stack.ts         # admin panel — ECS Fargate + 전용 ALB
│       ├── evaluation-stack.ts    # EX evaluator Lambda + Evaluator + OnlineEvalConfig + SSM bundle 포인터
...
│   └── datasource-admin-mcp/      # semantic 큐레이션·승인·채굴 + 데이터소스 관리 MCP (Manager/Admin 전용)
├── semantic-layer/                # DynamoDB CRUD·버전 관리(candidate/published/rejected) · seed · Streams→Neptune Lambda (uv)
├── evaluation/                    # Execution Accuracy(EX) code-based evaluator Lambda + goldset (uv)
...
# 3) 에이전트 컨테이너 이미지 4개 빌드·푸시 (ARM64, docker→finch 자동 폴백)
scripts/build-and-push.sh orchestrator sql-execution-mcp semantic-retrieval-mcp datasource-admin-mcp
...
# 5) Runtime 스택 배포 (AgentCore Runtime × 4 — semantic MCP 는 VPC 모드)
scripts/deploy.sh runtime
#    → infra/runtime-outputs.json 생성

# 6) Gateway 스택 배포 (Gateway + MCP target 3 + Cedar PolicyEngine ENFORCE) — 2-phase
scripts/deploy.sh gateway          # phase 1: 광역 permit 으로 target 등록·도구 동기화
scripts/deploy.sh gateway-scoped   # phase 2: 일반 사용자를 run_sql/search_schema 만 허용
#    → infra/gateway-outputs.json 생성 (GatewayUrl 출력)
#    ⚠️ Cedar action 목록 정책은 target 의 도구 동기화가 끝난 뒤에만 생성 검증을
#       통과하므로 반드시 phase 1 → phase 2 순서로 배포합니다.
...
# 7) Evaluation 스택 배포 (EX evaluator Lambda + Evaluator + OnlineEvalConfig
#    + SSM active-bundle 포인터) — admin 스택이 이 출력을 env 로 소비하므로 admin 전에 배포
scripts/deploy.sh evaluation
#    → infra/evaluation-outputs.json 생성

# 8) UI·admin panel 이미지 빌드·푸시 후 각 스택 배포
scripts/build-and-push.sh ui admin-web
scripts/deploy.sh ui
#    → infra/ui-outputs.json 생성 (AlbUrl 출력)
scripts/deploy.sh admin
#    → infra/admin-outputs.json 생성 (AdminAlbUrl 출력)

# 9) E2E 스모크 테스트 (레벨1~7: MCP · orchestrator SSE · UI · clarification ·
#    Gateway/Cedar/Redshift · admin panel 큐레이션·승인·OBO · 개선 파이프라인)
...
**개선 파이프라인 사용**: admin panel 의 **평가·개선** 화면에서 ① "후보 채굴 실행"으로 최근 운영 질의에서 few-shot/용어 후보를 채굴(→ 승인 큐 유입), ② 배치 평가를 실행해 EX·Correctness 스코어를 리뷰, ③ 개선 추천(시스템 프롬프트/도구 설명)을 생성해 bundle 새 버전으로 반영, ④ 원하는 bundle 버전을 **승격**하면 orchestrator 가 약 60초 내(캐시 TTL) 새 프롬프트·모델로 전환됩니다. 롤백은 이전 버전을 다시 승격하면 됩니다.

## 보안 참고 사항

- **READ-ONLY 4중 방어**: Data API는 DML/DDL도 실행 가능하므로 애플리케이션 레벨 강제가 필수입니다.
  1. **Cedar default-deny policy** — Gateway의 PolicyEngine(ENFORCE)이 도구 호출 자체를 인가. 미인가 도구는 tools/list에서 제외됩니다.
  2. **SQL AST validator** (SQLGlot allow-list) — LLM 밖 결정적 검증. SELECT/WITH만, 단일 statement, 시스템 카탈로그·위험 함수 차단, 자동 LIMIT 주입. (`sql-execution-mcp/validation.py`)
  3. **read-only IAM 역할** — MCP 실행 역할은 특정 클러스터 ARN에 대한 `rds-data`/`redshift-data` 실행 권한과 `agent_ro` 시크릿 읽기만 부여.
  4. **DB SELECT-only grant** — `agent_ro` DB 역할은 SELECT 권한만 보유(seed가 부여). 쓰기 권한 없음.
- **컴포넌트별 최소 권한 IAM 역할 분리**: orchestrator / sql-mcp / semantic-mcp / admin-mcp / ui / admin-web이 각각 별도 역할을 사용하며, 각 역할은 자신에게 필요한 액션만 특정 리소스 ARN에 한정해 보유합니다.
...
- **알려진 한계 (프로덕션 전 보완 필요)**:
  ...
  - **채팅 UI는 인증이 강제되지 않습니다.** UI 프록시(`/api/copilotkit`)에 Cognito JWT 검증 훅 자리만 마련돼 있습니다. admin panel은 Cognito 로그인 + JWT 검증 + 그룹 인가가 강제되며, 사용자 토큰이 Gateway MCP까지 OBO로 전파됩니다. orchestrator 경로의 사용자 토큰 전파는 additive로 구현돼 있으나(`forwardedProps.userAccessToken`) UI 로그인이 없어 기본은 서비스 계정 위임입니다.
