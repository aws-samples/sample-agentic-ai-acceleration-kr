...
#   2) [이미지 빌드·푸시: scripts/build-and-push.sh]  ← Base 완료 후, Runtime 전에 필수
#   3) [seed: scripts/seed.sh]                        ← Runtime 전에 데이터 적재
#   4) AgenticT2SqlRuntimeStack → runtime-outputs.json (AgentCore Runtime 3개)
#   5) [UI 이미지 빌드·푸시]
#   6) AgenticT2SqlUiStack      → ui-outputs.json     (ECS Fargate + ALB)
#
# 사용:
