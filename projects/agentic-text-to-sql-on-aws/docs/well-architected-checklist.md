...
- [x] **AGENTOPS04** — AgentCore Gateway = 권위 있는 tool catalog: 각 MCP 서버를 owner/version/param schema와 함께 등록, semantic discovery, 툴 fallback 정의 ✅ 구현: Gateway 가 유일한 도구 평면(semantic tool search 기본 활성). orchestrator 는 TOOL_PLANE_MODE=gateway 로 전환 가능(기본 배포값은 direct — gateway 전환은 README 의 update-agent-runtime 수동 절차). MCP target 3개(datasource-admin-mcp 추가 — 관리 도구도 동일 카탈로그·Cedar 인가 경로)
