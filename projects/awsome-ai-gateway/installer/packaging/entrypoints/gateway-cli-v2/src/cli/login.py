...
Reads OIDC config (issuer_url, client_id) from the resolved site config so the
end user doesn't have to set any environment variables. The full PKCE Authorization
Code flow runs locally:
...
def resolve_login_user_id() -> str | None:
    ...
    Prefers the cached OIDC id_token's ``email`` claim (matches the
    ``<user>@example.com`` format the operator expects), then falls back to
    ``preferred_username`` / ``cognito:username`` / ``sub``, then to the VK
    cache's ``user_id``. Returns None when the user has not logged in or no
    usable claim is present, so the caller can degrade gracefully.
    """
    tokens = load_tokens()
...
def _provision_vk_after_login(cfg: ResolvedConfig, tokens: Tokens, issuer_url: str) -> None:
    ...
    rejection. No-op when no admin_api_url is configured.
    """
    admin_api_url = cfg.admin_api_url.rstrip("/") if cfg.admin_api_url else ""
    if not admin_api_url:
        return
    device_name = socket.gethostname()
    id_token = tokens.id_token or tokens.access_token
    try:
        vk = _exchange_jwt_for_vk(admin_api_url, id_token, device_name, issuer_url=issuer_url)
...
def run_login_password(
    ...
    accounts) by prompting for a new password.
    """
    import click
    ...
    if not issuer_url or not client_id:
        raise LoginStepError("missing oidcIssuerUrl or oidcClientId.")
    ...
    save_tokens(tokens)
    _provision_vk_after_login(cfg, tokens, issuer_url)
...
def run_login(
    ...
    """Execute the OIDC PKCE browser flow using the resolved OIDC credentials.
    ...
    if admin_api_url:
        ...
        try:
            vk = _exchange_jwt_for_vk(
                admin_api_url, id_token, device_name, issuer_url=issuer_url
            )
            save_vk_cache(vk)
            log.info(
                "vk_cached",
