...
"""Binary removal for gateway-cli — delegate to the Inno uninstaller.

Why this exists
---------------
The installed suite (``gateway-cli.exe`` + ``api-key-helper.exe`` +
``statusline.exe`` + the shared PyInstaller ``_internal\\`` runtime) lives in one
directory under ``C:\\Program Files\\GatewayCLI``. A running exe holds an
**exclusive lock** on its own image and every DLL loaded from ``_internal\\``, so
the CLI can NEVER delete that directory from within its own process — an
in-process ``Remove-Item`` half-guts the folder and fails. The Inno
``unins000.exe`` escapes this by self-copying to ``%TEMP%`` and relaunching from
there; it then reverts the PATH entry and the Add/Remove-Programs (ARP)
registration and deletes the whole install dir.

So this module does exactly ONE thing: resolve the uninstaller and hand off to it
elevated + detached, then return. It never touches the install dir directly.
Software-level state (managed settings, user settings, OS env vars, tokens,
backups) is NOT this module's job — that is ``clear`` (see :mod:`cli.teardown`).
Because ``uninstall`` deletes the exe that ``clear`` runs from, run ``clear``
FIRST (the ``uninstall --clear-first`` flag does this).

Ported from the shipped Cowork implementation (``cowork_uninstall.py``) with this
product's identity anchors swapped in; the resolver/validator logic — including
the ARP double-brace quirk handling — is unchanged.

Scope
-----
Windows only. On macOS/Linux the CLI is delivered via the platform installer
(``packaging-linux``); those platforms report that binary removal is out of
scope here and point at the manual path.
"""
...
log = structlog.get_logger(component="uninstall")

# The Inno AppId (packaging/installer.iss) — the ARP/uninstall key is
# "<AppId>_is1" under the Uninstall hive, in HKLM (admin install) or HKCU
# (per-user install), in the native and WOW6432 views. We resolve the
# UninstallString rather than hardcoding a path so a per-user install is found too.
#
# We match on the bare GUID as a SUBSTRING of the subkey name (+ the "_is1"
# suffix) instead of reconstructing the full "{GUID}_is1" string, because
# installer.iss uses `AppId={{9B7C2E64-...}}` whose trailing `}}` is emitted
# verbatim by Inno — so the REAL key on disk is `{GUID}}_is1` (double close
# brace), not the conventional `{GUID}_is1`. Substring matching is robust to
# that brace quirk (and to a future .iss fix that normalises it). The same quirk
# was verified live for the sibling Cowork installer on the Windows test box
# (2026-08-10); this .iss uses the identical `{{...}}` pattern. Do NOT "fix" the
# .iss — changing AppId changes the product identity and orphans existing installs.
_APP_GUID = "9B7C2E64-5D1A-4F3B-8C0E-2A6F41D95B37"
_UNINSTALL_ROOTS = (
...
# our RunAs launch elevate it. We therefore additionally require the resolved
# path to be a real Inno uninstaller (`unins###.exe`) living inside our own,
# uniquely-named install directory (which also rejects the sibling Cowork CLI's
# uninstaller — different install dir).
...
_EXPECTED_DIR_NAME = "gatewaycli"  # compared case-insensitively
_EXPECTED_DISPLAY_NAME = "LLM Gateway CLI"
...
_MANUAL_HINT_WIN = (
    ...
    "    1. Settings -> Apps -> Installed apps -> 'LLM Gateway CLI' -> Uninstall\n"
    "       (or run C:\\Program Files\\GatewayCLI\\unins000.exe)\n"
...
_MANUAL_HINT_POSIX = (
    ...
    "gateway-cli package the same way you installed it."
)
...
def _run_powershell(script: str, timeout: int = 20) -> str:
    ...
    that fail to decode as UTF-8, masking the real error with mojibake. The
    prologue forces the output stream to UTF-8 and encoding=/errors= make the
    Python decode match + never crash.
    """
...
def _running_as_system_windows() -> bool:
    ...
    interactive UAC a real uninstall needs, and would target the wrong per-user
    hive — so ``uninstall`` refuses there.
    """
...
def _validate_uninstaller(entry: _ArpEntry) -> tuple[str | None, str | None]:
    ...
    OUR Inno uninstaller. Checks, all of which must pass:
      ...
      2. That file lives inside a directory whose leaf is our uniquely-named
         install dir (``GatewayCLI``). This both proves ownership and rejects
         the sibling Cowork CLI's uninstaller (different install dir).
    ...
    if path.parent.name.lower() != _EXPECTED_DIR_NAME:
        return None, (
            f"uninstaller is not in the gateway-cli install directory "
...
def uninstall(*, dry_run: bool = False) -> UninstallOutcome:
    ...
    ``/VERYSILENT /NORESTART`` elevated (interactive UAC). The uninstaller runs
    detached from ``%TEMP%`` and removes the three exes, the shared
    ``_internal\\`` runtime, the PATH entry, and the ARP key after this process
    exits.
    ...
    if not uninstall_string:
        return UninstallOutcome(
            False, True,
            "gateway-cli is not registered in Add/Remove Programs — nothing to "
